...
export default function FacePage() {
  ...
  useEffect(() => {
    const fetchFaceProducts = async () => {
      try {
        ...
        // Filter products that are face-related
        const faceProducts = allProducts.filter(product => 
          product.product_type?.toLowerCase().includes('foundation') ||
          product.product_type?.toLowerCase().includes('concealer') ||
          product.product_type?.toLowerCase().includes('blush') ||
          product.product_type?.toLowerCase().includes('bronzer') ||
          product.product_type?.toLowerCase().includes('highlighter') ||
          product.product_type?.toLowerCase().includes('powder') ||
          product.product_type?.toLowerCase().includes('primer') ||
          product.category?.toLowerCase().includes('face')
        );
        setFaceProducts(faceProducts);
      } catch (error) {
  ...
  return (
    <div className="w-full">
          ...
          <div className="absolute inset-0 flex items-center justify-end pr-20">
            <div className="text-right">
              ...
              <h2 className="text-7xl font-extrabold text-white flex items-center">
                FACE <span className="font-script ml-4 text-8xl">glow</span>
              </h2>
            </div>
          </div>
        </div>
      ...
      {/* Products Section */}
      <div className="p-6">
        <h1 className="text-3xl font-bold text-center mb-8">Face Products</h1>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {faceProducts.map((product) => (
            ...
            >
              <Link href={`/face/${product.id}`} passHref>
