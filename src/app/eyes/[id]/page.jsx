  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        // Strip HTML tags from name and description
        foundProduct.name = stripHtmlTags(foundProduct.name);
        foundProduct.description = stripHtmlTags(foundProduct.description);

        // Ensure price is a number
        foundProduct.price = Number(foundProduct.price) || 0;
        ...
        // Strip HTML tags from recommended products
        recommended.forEach(rec => {
          rec.name = stripHtmlTags(rec.name);
          rec.description = stripHtmlTags(rec.description);
  ...
  if (error || !product) {
    return (
      <div className="text-center p-6">
        ...
        <Link href="/eyes" className="text-pink-500 hover:underline mt-4 block">
          Back to Eyes
        </Link>
      </div>
    );
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <nav className="mb-8">
        <ol className="flex items-center space-x-2 text-sm">
          ...
          <li>
            <Link href="/eyes" className="text-gray-500 hover:text-pink-600">
              Eyes
            </Link>
          </li>
          <li>
        ...
        <div className="mt-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            {recommendedProducts.map((rec) => (
              <Link key={rec.id} href={`/eyes/${rec.id}`}>
